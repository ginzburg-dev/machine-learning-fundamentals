...
from typing import Tuple, Sequence, Any, List, Dict, NamedTuple
...
class DenoiserPatchDataset(Dataset):
    def __init__(
        ...
        self.samples = collect_clean_noisy_samples(
           root=self.input_dir,
           layer="chars",
...
def collect_clean_noisy_samples(
    root: Path,
    layer: str = "chars",                         # "env" или "chars"
    noise_levels: Sequence[str] = ("high",),
    subsets: Sequence[str] = ("anim",),
    aovs: Sequence[str] = (),                    # например ("albedo","normal","depth")
    ext: str = ".exr",
    n_first_samples: int | None = None,
    n_first_frames: int | None = None
) -> list[SampleInfo]:
